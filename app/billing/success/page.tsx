...
export default function BillingSuccessPage() {
  ...
  useEffect(() => {
    // Optionally: call backend or refresh user context here
    ...
    // Ask backend/session to refresh so dashboards immediately reflect plan
    try { refreshSession() } catch {}
    // Set provisional activation window (15 minutes) to avoid repeat upgrade prompts while syncing
    try {
    ...
    // Proactively trigger backend sync; ignore errors/rate limits and keep polling below
    ;(async () => {
      try {
    ...
    // Immediate redirect based on role param (webhook will unlock features)
    ...
    // Poll backend profile quickly for up to ~10s to detect activation
    let elapsed = 0
    const tick = async () => {
      try {
        // ensure session also refreshes cookies if needed
    ...
    const interval = setInterval(async () => {
      ...
      if (done) { clearInterval(interval) }
        ...
        if (elapsed >= 10000) {
          ...
          // Fallback: go to the likely dashboard; webhook will unlock shortly
          const role = (session as any)?.role
          router.push(role === 'recruiter' ? '/dashboard/recruiter' : role === 'job_seeker' ? '/dashboard/job-seeker' : '/')
        }
      }
    }, 500)
  ...
  return (
    <div className="mx-auto max-w-xl p-8 space-y-4">
      ...
      <p className="text-sm text-muted-foreground">{status}{nextDate ? ` • Next billing: ${nextDate}` : ''}</p>
      <div className="flex gap-3 pt-2">
        ...
        <button className="rounded border px-4 py-2" onClick={() => router.push("/billing")}>Manage Billing</button>
      </div>
    </div>
  )
}
