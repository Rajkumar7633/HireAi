...
export async function POST(req: NextRequest) {
  try {
    ...
    const del = await SocialConnection.findOneAndDelete({ requesterId, addresseeId: me, status: "pending" });
    if (!del) return NextResponse.json({ error: "Not found" }, { status: 404 });
    emitSocial("connection:update", { type: "declined", users: [String(requesterId), String(me)] });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("/api/social/connections/decline error", e);
